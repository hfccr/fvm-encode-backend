...
contract TokenRender {
    ...
    function image(
        ...
        // Check status
        if (timestamp_start > 0 && (timestamp_start + duration) > block.timestamp) {
            parts[0] = base("fff");
            status = "ACTIVE";
            parts[1] = "d2d2d2";
            parts[3] = "33ff94";
        ...
        parts[8] = substring(data_uri, bytes(data_uri).length - 15, bytes(data_uri).length);
        // Adding expiration
        if (days_left > 0) {
    ...
    function render(
        ...
        if (contains("ipfs://", data_uri)) {
            ...
            string memory svg = image(data_uri, timestamp_start, duration, appeal);
            json_parts = string(abi.encodePacked(json_parts, Base64.encode(bytes(svg))));
            // Appeal text
